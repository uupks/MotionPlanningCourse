...
class MpcCar {
 ...
 private:
  ...
  void linearization(const double& phi,
    ...
    Eigen::MatrixXd Ac(n, n);
    Eigen::MatrixXd Bc(n, m);
    Eigen::MatrixXd gc(n, 1);
    Eigen::MatrixXd iden = Eigen::MatrixXd::Identity(n, n);

    Ac << 0, 0, -v * sinphi, cosphi,
           0, 0, v * cosphi, sinphi,
           0, 0, 0, tan(delta) / ll_,
           0, 0, 0, 0;
    ...
    Bc << 0, 0,
           0, 0,
           0, v / (ll_ * cosdelta * cosdelta),
           1, 0;
    
    gc << v * phi * sinphi,
           -v * phi * cosphi,
           -v * delta / (ll_ * cosdelta * cosdelta),
           0;
 ...
 public:
  ...
  int solveQP(const VectorX& x0_observe) {
    ...
    printf("state : [px : %f, py : %f, phi : %f, v : %f]\n", x0(0), x0(1), x0(2), x0(3));
    // set BB, AA, gg
    ...
    for (int i = 0; i < N_; ++i) {
      ...
      // printf("i : %d, phi : %f, v : %f, delta: %f\n", i, phi, v, delta);
      linearization(phi, v, delta);
      // std::cout<<"Bd_ :\n"<<Bd_<<std::endl;
      // std::cout<<"Ad_ :\n"<<Ad_<<std::endl;
      // std::cout<<"gd_ :\n"<<gd_<<std::endl<<std::endl;
      
      // auto matrix_pow = [&](Eigen::MatrixXd M, int p) {
      //   Eigen::MatrixXd MP = M;
      //   MP.setIdentity();
      ...
      if (i == 0) {
        ...
        //   int p = i - j;
        //   auto Ap = matrix_pow(Ad_, p);
        //   BB.block(i * n, m * j, n, m) = Ap * Bd_;
        // }
        // auto Ai1 = matrix_pow(Ad_, i+1);
        // AA.block<n, n>(i * 4, 0) = Ai1;
        // gg.block<n, 1>(i * 4, 0) = gd_;
      }
